...
class AuthRepository implements AuthRepoInterface {
  ...
  Future<ApiResponse> login(
      String? userInput, String? password, String? guest) async {
    try {
      Response response = await dioClient!.post(
        AppConstants.loginUri,
        data: {"email": userInput, "password": password, "guest_id": guest},
      );
    //  print("______________________________________${Provider.of<AuthController>(Get.context!, listen: false).getGuestToken()}");
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
  ...
  Future<ApiResponse> verifyEmail(
    ...
    try {
      Response response =
          await dioClient!.post(AppConstants.verifyPhoneUrii, data: {
        "email": email,
        "token": token,
      });
      return ApiResponse.withSuccess(response);
  ...
  Future<ApiResponse> verifyPhone(
    ...
    print("===123456====>>${{"phone": phone.trim(), "code": otp}}");
    try {
      Response response = await dioClient!.post(AppConstants.verifyPhoneUrii,
          data: {"phone": phone.trim(), "code": otp});
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
